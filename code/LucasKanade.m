...
while metric_difference(i)>eps%% && i<50
    
    % warp I with W(x,p) to compute I(W(x,p))
    W = [1 0 delta_p(1); 0 1 delta_p(2)];
    warped_coords = (W*coords')'; % 
    Uq = reshape(warped_coords(:,2),[y,x])'; % make into equivalent meshgrids to above
...
%     b = It-I_w;
    ...
    A = delIcol_crop*J;    
        Ax = reshape(A(:,1),[yt,xt])'; % turn back to a matrix
        Ay = reshape(A(:,1),[yt,xt])'; % turn back to a matrix
...
%     p = p+delta_p;    
    ...
    metric_difference(i) = abs(metric-past_metric);
end
dp_x = delta_p(1);
dp_y = delta_p(2);
end